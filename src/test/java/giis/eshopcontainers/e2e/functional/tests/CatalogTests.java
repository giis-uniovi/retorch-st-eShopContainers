...
class CatalogTests extends BaseLoggedClass {
    ...
    public void selectFilter(String filterId, String[] filterOptions, Integer option) throws ElementNotFoundException {
        waiter.waitUntil(ExpectedConditions.elementToBeClickable(By.id(filterId)), filterId + " select not clickable");
        new Select(driver.findElement(By.id(filterId))).selectByIndex(option - 1);
