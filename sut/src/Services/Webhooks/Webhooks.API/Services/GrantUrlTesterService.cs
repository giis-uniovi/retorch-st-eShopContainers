...
class GrantUrlTesterService : IGrantUrlTesterService
{
    ...
    private readonly ILogger<GrantUrlTesterService> _logger;
    public GrantUrlTesterService(IHttpClientFactory factory, ILogger<GrantUrlTesterService> logger)
    {
    ...
    public async Task<bool> TestGrantUrl(string urlHook, string url, string token)
    {
        ...
        catch (Exception ex)
        {
            if (_logger.IsEnabled(LogLevel.Warning))
                _logger.LogWarning(ex, "Exception {TypeName} when sending OPTIONS request. Url can't be granted.", ex.GetType().Name);
            return false;
        }
    }
